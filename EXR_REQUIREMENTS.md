...
**Confirmed: Flame writes true multi-part EXR — and its AOVs don't use the dotted `<layer>.<channel>` convention at all.** A real Flame render (beauty + depth + normals + Cryptomatte, verified with `oiiotool --info -v -a`) came back with AOVs zero-filled/black in ComfyUI despite the source data being present. Two things were wrong with the assumption above, not one: (1) each layer lands in its own EXR subimage/part rather than one multichannel part, so plain OIIO `ImageInput.open()`/`spec()`/`read_image()` (which only ever see subimage 0) never found them; and (2) every part just uses generic `R,G,B` channel names — there is no `N.X`/`Z` anywhere in the file. The AOV's identity is carried entirely by the EXR part's own `name` attribute (`"depth"`, `"normals"`, `"cryptomatte"`, ...). Root cause: these AOVs were imported into Flame from another DCC (Cinema 4D), not generated by Flame itself, so the dotted-layer convention this doc originally assumed (matching `generate_synthetic_exr.py`) never applied to that data — it was never a Flame convention to begin with, just this project's own synthetic-fixture convention.

`ComfyUI_OpenClip/lib/image_io.py` now supports both, tried independently per AOV: the dotted-channel convention above (on subimage 0 only), and a **part-name convention** — new Reader inputs `depth_part_name`/`normal_world_part_name`/`normal_raw_part_name` (`STRING`, e.g. defaulting to `"depth"`/`""`/`"normals"` respectively) match against each subimage's `name` attribute; only a match gets pixel-decoded, so an unmatched part (Cryptomatte, in particular — its channels are *also* just `R,G,B`, indistinguishable from a real AOV by channel name alone) never pays the potentially-large decode cost. Depth's data was found to be a replicated scalar across `R,G,B` — channel 0 is read as the value. Critically, **this repo does not try to determine whether an ambiguous normals-like part is camera-space or world-space** (there was no way to know from the file — the data didn't originate in Flame); that part is exposed as a new `NORMAL_RAW` output distinct from the space-committed `NORMAL`/`NORMAL_WORLD`, and space labelling is left to a Harmonize-side node/toggle.
